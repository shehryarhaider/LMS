    ...
    <div class="portlet">
      <div class="portlet-heading bg-light-theme">
        <h3 class="portlet-title">
          <i class="ti-harddrives mr-2"></i> Controller Files</h3>
        <div class="portlet-widgets">
          <span class="divider"></span>
          <a href="{{route('vendor.mvc.directory',['controller'])}}" class="btn btn-white btn-custom btn-custom-white btn-rounded waves-effect">
            <i class="fa fa-save mr-1"></i> Add Directory
          </a>
          <span class="divider"></span>
          <a href="{{route('vendor.mvc.controller.create')}}" class="btn btn-white btn-custom btn-custom-white btn-rounded waves-effect">
            <i class="fa fa-folder mr-1"></i> Add Controller
          </a>
        </div>
  ...
  $(document).ready(function () {
    ...
    $(document).on('click', '.delete', function () {
      ...
      swal({
        ...
        if (result.value) {
          axios
            .post('{{route("vendor.mvc.controller.destroy")}}', {
              _method: 'delete',
              _token: '{{csrf_token()}}',
              name: deleteName,
            })
            .then(function (response) {
              ...
              swal(
                'Deleted!',
                'Your Controller has been deleted.',
                'success'
              )
              table
                .row($this.parents('tr'))
