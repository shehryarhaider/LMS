    ...
    <div class="portlet">
      <div class="portlet-heading bg-light-theme">
        <h3 class="portlet-title">
          <i class="ti-sharethis mr-2"></i> Blog Category</h3>
          ...
          @if ( $permissions == "is_admin" || in_array( 'add', $permissions ) )
            <a href="{{route('blog.category.create')}}">
              <button class="btn btn-white btn-custom-white btn-custom btn-rounded waves-effect" type="button">
                <i class="fa fa-plus"></i> Add Blog Category</button>
            </a>
          @endif
        </div>
...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    var table = $('#datatable').DataTable({
      processing: true,
      serverSide: true,
      ajax: '{{ route("blog.category.datatable") }}',
      "columns": [
      ...
      @if ($count > 0)
      {
        "targets": -1,
        "render": function (data, type, row, meta) {
          var edit = '{{route("blog.category.edit",[":id"])}}';
      ...
      "drawCallback": function (settings) {
        ...
        $('.status').change(function () {
          ...
          axios
            .post('{{route("blog.category.status")}}', {
              _token: '{{csrf_token()}}',
              _method: 'patch',
              id: id,
              status: status,
        ...
        $('.delete').click(function () {
          ...
          swal({
            ...
            if (result.value) {
            axios
              .post('{{route("blog.category.destroy")}}', {
                _method: 'delete',
                _token: '{{csrf_token()}}',
                id: deleteId,
              })
