    ...
    <div class="portlet">
      <div class="portlet-heading bg-light-theme">
        <h3 class="portlet-title">
          <i class="ti-sharethis mr-2"></i> Event Category</h3>
          ...
          @if ( $permissions == "is_admin" || in_array( 'add', $permissions ) )
            <a href="{{route('event_categories.create')}}">
              <button class="btn btn-white btn-custom-white btn-custom btn-rounded waves-effect" type="button">
                <i class="fa fa-plus"></i> Add Event Category</button>
            </a>
          @endif
        </div>
...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    var table = $('#datatable').DataTable({
      processing: true,
      serverSide: true,
      ajax: '{{ route("event_categories.datatable") }}',
      "columns": [
      ...
      @if ($count > 0)
      {
        "targets": -1,
        "render": function (data, type, row, meta) {
          var edit = '{{route("event_categories.edit",[":id"])}}';
      ...
      "drawCallback": function (settings) {
        ...
        $('.status').change(function () {
          ...
          axios
            .post('{{route("event_categories.status")}}', {
              _token: '{{csrf_token()}}',
              _method: 'patch',
              id: id,
              status: status,
        ...
        $('.delete').click(function () {
          ...
          swal({
            ...
            if (result.value) {
            axios
              .post('{{route("event_categories.destroy")}}', {
                _method: 'delete',
                _token: '{{csrf_token()}}',
                id: deleteId,
              })
