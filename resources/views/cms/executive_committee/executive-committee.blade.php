      ...
      <div class="col-sm-12">
        ...
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-ite active">
            <a href="{{route('executive_committee',[Session::get("term_id")])}}">Executive Committee</a>
          </li>
        </ol>
      </div>
    </div>
    ...
    <div class="portlet">
      <div class="portlet-heading bg-light-theme">
        <h3 class="portlet-title">
          <i class="ti-user mr-2"></i> Executive Committee</h3>
        <div class="portlet-widgets">
            @if ( $permissions == "is_admin" || in_array( 'add', $permissions ) )
         <a href="{{route('executive_committee.create',[Session::get('term_id')])}}">
            <button class="btn btn-white btn-custom-white btn-custom btn-rounded waves-effect" type="button">
              <i class="fa fa-plus"></i> Add Executive Committee</button>
          </a>
            @endif
        </div>
...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    var table = $('#datatable').DataTable({
      processing: true,
      serverSide: true,
      ajax: '{{ route("executive_committee.datatable",[Session::get("term_id")]) }}',
      "columns": [
      ...
      @if ($count > 0)
      {
        "targets": -1,
        "render": function (data, type, row, meta) {
          var edit = '{{route("executive_committee.edit",[Session::get("term_id"),":id"])}}';
      ...
      "drawCallback": function (settings) {
        ...
        $('.status').change(function () {
          ...
          axios
            .post('{{route("executive_committee.status")}}', {
              _token: '{{csrf_token()}}',
              _method: 'patch',
              id: id,
              status: status,
        ...
        $('.delete').click(function () {
          ...
          swal({
            ...
            if (result.value) {
            axios
              .post('{{route("executive_committee.destroy")}}', {
                _method: 'delete',
                _token: '{{csrf_token()}}',
                id: deleteId,
              })
